...
> **关于用 https 提供素材**：ChatGPT 能渲染 `https://` 图片，因此把素材库放到任意静态托管
> 或隧道后面，再把技能里的地址前缀换成它即可。注意此时素材实际存放在那个地址上——**若素材
> 并非你有权公开分发的内容，务必做成私有或带访问控制**。它的 MCP 不是这里该走的路：那只能
> 连远程 HTTPS 服务（不支持本地 stdio），还需要付费套餐并开启开发者模式。
