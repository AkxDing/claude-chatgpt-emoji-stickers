...
- **ChatGPT 桌面端** —— `view_image({ path: "…/happy/wave.gif" })`，它的内置看图工具。
  2026-09-20 实测：直接读取本地文件并在会话里显示，**GIF 会循环播放**（与 Claude Code
  一致），而且**不需要授予任何文件访问权限**，开箱即用。不需要额外依赖、不需要服务、
...
| **ChatGPT 桌面端** | **方式 A 可用** —— 用它内置的 `view_image` 工具直接读本地路径，2026-09-20 实测：GIF 会循环播放、无需任何授权，**首选这条**。方式 B 也可用，但只限 https：助手输出的 `https://` 图片会内联显示，`http://127.0.0.1` 不行 |
| 纯终端 | 只会看到文件卡片或一条链接 |
