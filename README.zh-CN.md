...
桌面端实测通过，并且**任何会在回复里渲染图片的客户端都能用**——判断方法见
[我的客户端能用吗](#我的客户端能用吗)，一分钟就能自测。
...
**任何客户端都可以一分钟自测**：把服务跑起来，然后**让助手原样输出**这一行（不要放进代码块）——
...
| **ChatGPT 桌面端** | **方式 B 走 https 可用** —— 2026-09-20 实测：助手输出的 `https://` 图片会内联显示。方式 A 不可用（没有文件投递工具） |
| 纯终端 | 只会看到文件卡片或一条链接 |
...
### 用 https 提供素材，而且完全不用自己搭服务

客户端自身走 https 时可能拒绝 `http://127.0.0.1`（混合内容；且自 Chrome 145 起，访问本地
回环地址还要过一道权限提示），**所以这类客户端需要一个 https 地址**。但你不需要托管任何
东西：

**把素材库放进一个公开的 GitHub 仓库，直接用它的 raw 地址当前缀。**

```
https://raw.githubusercontent.com/<你>/<仓库>/main/stickers
https://cdn.jsdelivr.net/gh/<你>/<仓库>@main/stickers      # 同样的文件，走 CDN
```

两者都是普通 https，零成本、零运维。**本仓库的素材本身就可以这样访问**，所以你可以先直接
指向它把流程跑通，再换成自己收集的。

⚠️ **这等于把素材公开了。** 对于你有权分发的素材，这是最省事的正解；但对于从网上收集的
反应动图，这是错的——那种情况请改用私有隧道指向本地服务，或者干脆走方式 A。

> ChatGPT 的 MCP 不是这里该走的路：它只能连远程 HTTPS 服务（不支持本地 stdio），还需要
> 付费套餐并开启开发者模式。
